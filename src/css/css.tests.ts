...
it('&:pseudo', () => {
	const link = css({
		'root': {
			color: 'black',
			'&:hover': {color: 'green'},
			'&:first-child': {color: '#c00'},
			'&:not(:disabled)': { color: 'blue'},
			'&:not(.active)': { color: '#f60'},
		},
		'active': {
			color: 'red',
		},
	...
	expect(getUsedCSS(true).cssText.trim().split('\n')).toEqual([
		`.${link.root}:hover{color:green;}`,
		`.${link.root}:first-child{color:#c00;}`,
		`.${link.root}:not(:disabled){color:blue;}`,
		`.${link.root}:not(.${link.active}){color:#f60;}`,
